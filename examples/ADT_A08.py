...
from hl7.v2_5_1.segments import (
    AL1, DB1, PDA, UB2, OBX, UB1, NK1, SFT, PV1, MSH, ACC, PD1, DG1, IN2, IN3, ROL, PR1, DRG, GT1, PV2, IN1, EVN, PID
)
from hl7.v2_5_1.data_types import (
    AUI, TX, IS, JCC, SI, XON, MSG, OSP, DDI, ID, DR, XTN, FC, VID, PTA, OCD, CX, DTM, PT, TS, PL, UVC, CNE, MO, EI, HD, CP, NM, ICD, DLN, CWE, DLD, MOP, XAD, SAD, FN, RMC, XCN, XPN, DTN, CE, ST, DT, VARIES
)
from hl7.v2_5_1.tables import (
    CurrencyCodes, InsuranceCompanyContactReason, AlternateCharacterSets, MilitaryService, FirstName, Race, NatureOfAbnormalTesting, ZipCode, PurgeStatusCode, PatientStatusCode, AbnormalFlags, CpRangeType, OrganizationUnitType, TypeOfAgreement, JobStatus, DrgPayor, MailClaimParty, CheckDigitScheme, VisitIndicator, OrganizationalNameType, RoomType, AdvanceDirectiveCode, AdmitSource, MilitaryStatus, CoordinationOfBenefits, ProcedurePriority, TelecommunicationUseCode, AdmissionType, CodingSystem, ProcessingMode, PriceType, OrganDonorCode, ContactRole, MessageStructure, AmountType, NameAssemblyOrder, PersonLocationType, NotifyClergyCode, VisitUserCode, HospitalService, State, ProcedureDrgType, Religion, YesOrNoIndicator, AlternateCharacterSetHandlingScheme, AllergySeverity, ObservationResultStatusCodesInterpretation, MilitaryRankOrGrade, AdministrativeSex, UniversalIdType, NameOrAddressRepresentation, EthnicGroup, VisitPriorityCode, AdmissionLevelOfCareCode, AllergenType, StudentStatus, ModeOfArrivalCode, CertificationPatientType, StateOrProvince, DayType, Relationship, AmountClass, CountryCode, Lastname, ReAdmissionIndicator, CoverageType, SignatureCode, ProblemOrGoalActionCode, BedStatus, EventType, PatientClass, DischargeDisposition, ProcessingId, LivingDependency, PolicyType, NameType, City, PublicityCode, TelecommunicationEquipmentType, DrgTransferType, TissueTypeCode, VersionId, EmploymentStatus, ReleaseInformation, AssignmentOfBenefits, LivingArrangement, ValueType, ProcedureFunctionalType, PrecautionCode, ProductionClassCode, MoneyOrPercentageIndicator, ProviderRole, DiagnosisType, Precision, DiagnosisClassification, SegmentActionCode, PatientSRelationshipToInsured, LivingWillCode, EligibilitySource, ImmunizationRegistryStatus, RecreationalDrugUseCode, DegreeOrLicenseOrCertificate, MessageType, AcceptOrApplicationAcknowledgmentConditions, AddressType, PatientConditionCode, SpecialProgramCode, DiagnosisPriority, IdentifierType, MaritalStatus, IdentityReliabilityCode, AmbulatoryStatus, OutlierType, DisabledPersonCode, EventReason
)
# fmt: on
...
async def msh_1(x: UpstreamInput) -> MSH:
    ...
    msh = MSH(
        ...
        sending_application=HD(
            namespace_id=IS(x),
            universal_id=ST(x),
            universal_id_type=UniversalIdType.parse(x, using=llm_parser),
        ),
        sending_facility=HD(
            namespace_id=IS(x),
            universal_id=ST(x),
        ...
        date_or_time_of_message=TS(
            time=DTM(x), degree_of_precision=Precision.parse(x, using=llm_parser)
        ),
        security=ST(x),
        message_type=MSG(
            message_code=MessageType.parse(x, using=llm_parser),
            trigger_event=EventType.parse(x, using=llm_parser),
            message_structure=MessageStructure.parse(x, using=llm_parser),
        ),
        message_control_id=ST(x),
        processing_id=PT(
            processing_id=ProcessingId.parse(x, using=llm_parser),
            processing_mode=ProcessingMode.parse(x, using=llm_parser),
        ),
        version_id=VID(
            version_id=VersionId.parse(x, using=llm_parser),
            internationalization_code=CountryCode.parse(x, using=llm_parser),
            international_version_id=CE(
                identifier=ST(x),
                text=ST(x),
                name_of_coding_system=CodingSystem.parse(x, using=llm_parser),
                alternate_identifier=ST(x),
                alternate_text=ST(x),
                name_of_alternate_coding_system=CodingSystem.parse(x, using=llm_parser),
            ),
        ),
        ...
        country_code=CountryCode.parse(x, using=llm_parser),
        character_set=AlternateCharacterSets.parse(x, using=llm_parser),
...
async def sft_2(x: UpstreamInput) -> SFT:
    ...
    sft = SFT(
        software_vendor_organization=XON(
            ...
            id_number=NM(x),
            check_digit=NM(x),
            check_digit_scheme=CheckDigitScheme.parse(x, using=llm_parser),
            assigning_authority=HD(
                namespace_id=IS(x),
                universal_id=ST(x),
                universal_id_type=UniversalIdType.parse(x, using=llm_parser),
            ),
            identifier_type_code=IdentifierType.parse(x, using=llm_parser),
...
async def evn_3(x: UpstreamInput) -> EVN:
    ...
    """
    evn = EVN(
        event_type_code=EventType.parse(x, using=llm_parser),
        ...
        event_reason_code=EventReason.parse(x, using=llm_parser),
        operator_id=XCN(
            id_number=ST(x),
            family_name=FN(
                surname=Lastname.parse(x, using=llm_parser),
                own_surname_prefix=ST(x),
                own_surname=ST(x),
                surname_prefix_from_partner_or_spouse=ST(x),
                surname_from_partner_or_spouse=ST(x),
            ),
            given_name=FirstName.parse(x, using=llm_parser),
            second_and_further_given_names_or_initials_thereof=ST(x),
            suffix=ST(x),
            prefix=ST(x),
            degree=DegreeOrLicenseOrCertificate.parse(x, using=llm_parser),
            source_table=IS(x),
            assigning_authority=HD(
                namespace_id=IS(x),
                universal_id=ST(x),
            ...
            name_type_code=NameType.parse(x, using=llm_parser),
            ...
            name_validity_range=DR(
                range_start_date_or_time=TS(
                    time=DTM(x),
                    degree_of_precision=Precision.parse(x, using=llm_parser),
                ),
            ...
            name_assembly_order=NameAssemblyOrder.parse(x, using=llm_parser),
            effective_date=TS(
                time=DTM(x), degree_of_precision=Precision.parse(x, using=llm_parser)
            ),
            expiration_date=TS(
...
async def pid_4(x: UpstreamInput) -> PID:
    ...
    pid = PID(
        ...
        administrative_sex=AdministrativeSex.parse(x, using=llm_parser),
        patient_alias=XPN(
            family_name=FN(
                surname=Lastname.parse(x, using=llm_parser),
                own_surname_prefix=ST(x),
        ...
        race=Race.parse(x, using=llm_parser),
        patient_address=XAD(
            street_address=SAD(
            ...
            city=City.parse(x, using=llm_parser),
            state_or_province=State.parse(x, using=llm_parser),
            zip_or_postal_code=ZipCode.parse(x, using=llm_parser),
            country=CountryCode.parse(x, using=llm_parser),
            address_type=AddressType.parse(x, using=llm_parser),
        ...
        marital_status=MaritalStatus.parse(x, using=llm_parser),
        religion=Religion.parse(x, using=llm_parser),
        ...
        ethnic_group=EthnicGroup.parse(x, using=llm_parser),
        birth_place=ST(x),
        multiple_birth_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        birth_order=NM(x),
        citizenship=CE(
            identifier=ST(x),
            text=ST(x),
        ...
        patient_death_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        identity_unknown_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        identity_reliability_code=IdentityReliabilityCode.parse(x, using=llm_parser),
        ...
        strain=ST(x),
        production_class_code=ProductionClassCode.parse(x, using=llm_parser),
...
async def pd1_5(x: UpstreamInput) -> PD1:
    ...
    """
    pd1 = PD1(
        living_dependency=LivingDependency.parse(x, using=llm_parser),
        living_arrangement=LivingArrangement.parse(x, using=llm_parser),
        ...
        student_indicator=StudentStatus.parse(x, using=llm_parser),
        handicap=IS(x),
        living_will_code=LivingWillCode.parse(x, using=llm_parser),
        organ_donor_code=OrganDonorCode.parse(x, using=llm_parser),
        separate_bill=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        publicity_code=PublicityCode.parse(x, using=llm_parser),
        protection_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        advance_directive_code=AdvanceDirectiveCode.parse(x, using=llm_parser),
        ...
        military_branch=MilitaryService.parse(x, using=llm_parser),
        military_rank_or_grade=MilitaryRankOrGrade.parse(x, using=llm_parser),
        military_status=MilitaryStatus.parse(x, using=llm_parser),
    )
...
async def rol_6(x: UpstreamInput) -> ROL:
    ...
    rol = ROL(
        ...
        action_code=ProblemOrGoalActionCode.parse(x, using=llm_parser),
        role_rol=ProviderRole.parse(x, using=llm_parser),
        role_person=XCN(
            id_number=ST(x),
            family_name=FN(
                surname=Lastname.parse(x, using=llm_parser),
        ...
        organization_unit_type=OrganizationUnitType.parse(x, using=llm_parser),
...
async def nk1_7(x: UpstreamInput) -> NK1:
    ...
    nk1 = NK1(
        ...
        relationship=Relationship.parse(x, using=llm_parser),
        address=XAD(
            street_address=SAD(
                street_or_mailing_address=ST(x),
                street_name=ST(x),
        ...
        contact_role=ContactRole.parse(x, using=llm_parser),
        start_date=DT(x),
        end_date=DT(x),
        next_of_kin_or_associated_parties_job_title=ST(x),
        next_of_kin_or_associated_parties_job_code_or_class=JCC(
            job_code=IS(x), job_class=IS(x), job_description_text=TX(x)
        ),
        ...
        living_dependency=LivingDependency.parse(x, using=llm_parser),
        ambulatory_status=AmbulatoryStatus.parse(x, using=llm_parser),
        citizenship=CE(
            identifier=ST(x),
            text=ST(x),
            name_of_coding_system=CodingSystem.parse(x, using=llm_parser),
        ...
        job_status=JobStatus.parse(x, using=llm_parser),
        race=Race.parse(x, using=llm_parser),
        handicap=IS(x),
...
async def pv1_8(x: UpstreamInput) -> PV1:
    ...
    pv1 = PV1(
        set_id_pv1=SI(x),
        patient_class=PatientClass.parse(x, using=llm_parser),
        assigned_patient_location=PL(
            ...
            person_location_type=PersonLocationType.parse(x, using=llm_parser),
            building=IS(x),
            floor=IS(x),
        ...
        admission_type=AdmissionType.parse(x, using=llm_parser),
        preadmit_number=CX(
            id_number=ST(x),
            check_digit=ST(x),
            check_digit_scheme=CheckDigitScheme.parse(x, using=llm_parser),
        ...
        hospital_service=HospitalService.parse(x, using=llm_parser),
        ...
        re_admission_indicator=ReAdmissionIndicator.parse(x, using=llm_parser),
        admit_source=AdmitSource.parse(x, using=llm_parser),
        ambulatory_status=AmbulatoryStatus.parse(x, using=llm_parser),
        vip_indicator=IS(x),
        ...
        discharge_disposition=DischargeDisposition.parse(x, using=llm_parser),
        ...
        bed_status=BedStatus.parse(x, using=llm_parser),
        account_status=IS(x),
        pending_location=PL(
            point_of_care=IS(x),
            room=IS(x),
        ...
        visit_indicator=VisitIndicator.parse(x, using=llm_parser),
...
async def pv2_9(x: UpstreamInput) -> PV2:
    ...
    pv2 = PV2(
        ...
        visit_user_code=VisitUserCode.parse(x, using=llm_parser),
        ...
        purge_status_code=PurgeStatusCode.parse(x, using=llm_parser),
        purge_status_date=DT(x),
        special_program_code=SpecialProgramCode.parse(x, using=llm_parser),
        retention_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        expected_number_of_insurance_plans=NM(x),
        visit_publicity_code=PublicityCode.parse(x, using=llm_parser),
        visit_protection_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        patient_status_code=PatientStatusCode.parse(x, using=llm_parser),
        visit_priority_code=VisitPriorityCode.parse(x, using=llm_parser),
        previous_treatment_date=DT(x),
        expected_discharge_disposition=DischargeDisposition.parse(x, using=llm_parser),
        ...
        billing_media_code=YesOrNoIndicator.parse(x, using=llm_parser),
        expected_surgery_date_and_time=TS(
            time=DTM(x), degree_of_precision=Precision.parse(x, using=llm_parser)
        ),
        military_partnership_code=YesOrNoIndicator.parse(x, using=llm_parser),
        military_non_availability_code=YesOrNoIndicator.parse(x, using=llm_parser),
        newborn_baby_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        baby_detained_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        mode_of_arrival_code=ModeOfArrivalCode.parse(x, using=llm_parser),
        recreational_drug_use_code=RecreationalDrugUseCode.parse(x, using=llm_parser),
        admission_level_of_care_code=AdmissionLevelOfCareCode.parse(
            x, using=llm_parser
        ),
        precaution_code=PrecautionCode.parse(x, using=llm_parser),
        patient_condition_code=PatientConditionCode.parse(x, using=llm_parser),
        living_will_code=LivingWillCode.parse(x, using=llm_parser),
        organ_donor_code=OrganDonorCode.parse(x, using=llm_parser),
        advance_directive_code=AdvanceDirectiveCode.parse(x, using=llm_parser),
        ...
        notify_clergy_code=NotifyClergyCode.parse(x, using=llm_parser),
    )
...
async def db1_11(x: UpstreamInput) -> DB1:
    ...
    db1 = DB1(
        set_id_db1=SI(x),
        disabled_person_code=DisabledPersonCode.parse(x, using=llm_parser),
        ...
        disabled_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
...
async def obx_12(x: UpstreamInput) -> OBX:
    ...
    obx = OBX(
        set_id_obx=SI(x),
        value_type=ValueType.parse(x, using=llm_parser),
        ...
        abnormal_flags=AbnormalFlags.parse(x, using=llm_parser),
        probability=NM(x),
        nature_of_abnormal_test=NatureOfAbnormalTesting.parse(x, using=llm_parser),
...
async def al1_13(x: UpstreamInput) -> AL1:
    ...
    al1 = AL1(
        set_id_al1=SI(x),
        allergen_type_code=AllergenType.parse(x, using=llm_parser),
        ...
        allergy_severity_code=AllergySeverity.parse(x, using=llm_parser),
...
async def dg1_14(x: UpstreamInput) -> DG1:
    ...
    dg1 = DG1(
        ...
        diagnosis_type=DiagnosisType.parse(x, using=llm_parser),
        ...
        drg_approval_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        drg_grouper_review_code=IS(x),
        outlier_type=OutlierType.parse(x, using=llm_parser),
        outlier_days=NM(x),
        outlier_cost=CP(
            price=MO(
                quantity=NM(x), denomination=CurrencyCodes.parse(x, using=llm_parser)
            ),
            price_type=PriceType.parse(x, using=llm_parser),
            from_value=NM(x),
            to_value=NM(x),
            range_units=CE(
                identifier=ST(x),
            ...
            range_type=CpRangeType.parse(x, using=llm_parser),
        ),
        grouper_version_and_type=ST(x),
        diagnosis_priority=DiagnosisPriority.parse(x, using=llm_parser),
        ...
        diagnosis_classification=DiagnosisClassification.parse(x, using=llm_parser),
        confidential_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        diagnosis_action_code=SegmentActionCode.parse(x, using=llm_parser),
    )
...
async def drg_15(x: UpstreamInput) -> DRG:
    ...
    drg = DRG(
        ...
        drg_payor=DrgPayor.parse(x, using=llm_parser),
        ...
        confidential_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        drg_transfer_type=DrgTransferType.parse(x, using=llm_parser),
    )
...
async def pr1_16(x: UpstreamInput) -> PR1:
    ...
    pr1 = PR1(
        ...
        procedure_functional_type=ProcedureFunctionalType.parse(x, using=llm_parser),
        ...
        procedure_priority=ProcedurePriority.parse(x, using=llm_parser),
        ...
        procedure_drg_type=ProcedureDrgType.parse(x, using=llm_parser),
        tissue_type_code=TissueTypeCode.parse(x, using=llm_parser),
        ...
        procedure_action_code=SegmentActionCode.parse(x, using=llm_parser),
    )
...
async def gt1_18(x: UpstreamInput) -> GT1:
    ...
    gt1 = GT1(
        ...
        guarantor_administrative_sex=AdministrativeSex.parse(x, using=llm_parser),
        guarantor_type=IS(x),
        guarantor_relationship=Relationship.parse(x, using=llm_parser),
        guarantor_ssn=ST(x),
        ...
        guarantor_employment_status=EmploymentStatus.parse(x, using=llm_parser),
        ...
        guarantor_billing_hold_flag=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        guarantor_death_flag=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        guarantor_marital_status_code=MaritalStatus.parse(x, using=llm_parser),
        ...
        contact_relationship=Relationship.parse(x, using=llm_parser),
        job_title=ST(x),
        ...
        guarantor_race=Race.parse(x, using=llm_parser),
...
async def in1_19(x: UpstreamInput) -> IN1:
    ...
    in1 = IN1(
        ...
        authorization_information=AUI(
            authorization_number=ST(x), date=DT(x), source=ST(x)
        ),
        plan_type=IS(x),
        name_of_insured=XPN(
            family_name=FN(
        ...
        insureds_relationship_to_patient=Relationship.parse(x, using=llm_parser),
        ...
        assignment_of_benefits=AssignmentOfBenefits.parse(x, using=llm_parser),
        coordination_of_benefits=CoordinationOfBenefits.parse(x, using=llm_parser),
        coord_of_ben_priority=ST(x),
        notice_of_admission_flag=YesOrNoIndicator.parse(x, using=llm_parser),
        notice_of_admission_date=DT(x),
        report_of_eligibility_flag=YesOrNoIndicator.parse(x, using=llm_parser),
        report_of_eligibility_date=DT(x),
        release_information_code=ReleaseInformation.parse(x, using=llm_parser),
        pre_admit_cert=ST(x),
        ...
        type_of_agreement_code=TypeOfAgreement.parse(x, using=llm_parser),
        billing_status=IS(x),
        ...
        insureds_employment_status=EmploymentStatus.parse(x, using=llm_parser),
        insureds_administrative_sex=AdministrativeSex.parse(x, using=llm_parser),
        ...
        coverage_type=CoverageType.parse(x, using=llm_parser),
        handicap=IS(x),
        ...
        signature_code=SignatureCode.parse(x, using=llm_parser),
...
async def in2_20(x: UpstreamInput) -> IN2:
    ...
    in2 = IN2(
        ...
        mail_claim_party=MailClaimParty.parse(x, using=llm_parser),
        ...
        military_service=MilitaryService.parse(x, using=llm_parser),
        military_rank_or_grade=MilitaryRankOrGrade.parse(x, using=llm_parser),
        military_status=MilitaryStatus.parse(x, using=llm_parser),
        military_retire_date=DT(x),
        military_non_avail_cert_on_file=YesOrNoIndicator.parse(x, using=llm_parser),
        baby_coverage=YesOrNoIndicator.parse(x, using=llm_parser),
        combine_baby_bill=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        eligibility_source=EligibilitySource.parse(x, using=llm_parser),
        room_coverage_type_or_amount=RMC(
            room_type=RoomType.parse(x, using=llm_parser),
            amount_type=AmountType.parse(x, using=llm_parser),
            ...
            money_or_percentage=MOP(
                ...
                currency_denomination=CurrencyCodes.parse(x, using=llm_parser),
            ),
        ),
        policy_type_or_amount=PTA(
            policy_type=PolicyType.parse(x, using=llm_parser),
            amount_class=AmountClass.parse(x, using=llm_parser),
            money_or_percentage_quantity=NM(x),
            money_or_percentage=MOP(
                money_or_percentage_indicator=MoneyOrPercentageIndicator.parse(
                    x, using=llm_parser
        ...
        guarantors_relationship_to_insured=Relationship.parse(x, using=llm_parser),
        ...
        suspend_flag=YesOrNoIndicator.parse(x, using=llm_parser),
        copay_limit_flag=YesOrNoIndicator.parse(x, using=llm_parser),
        stoploss_limit_flag=YesOrNoIndicator.parse(x, using=llm_parser),
...
async def in3_21(x: UpstreamInput) -> IN3:
    ...
    in3 = IN3(
        ...
        certification_required=YesOrNoIndicator.parse(x, using=llm_parser),
        penalty=MOP(
            money_or_percentage_indicator=MoneyOrPercentageIndicator.parse(
                x, using=llm_parser
            ),
        ...
        days=DTN(day_type=DayType.parse(x, using=llm_parser), number_of_days=NM(x)),
...
async def acc_23(x: UpstreamInput) -> ACC:
    ...
    acc = ACC(
        ...
        auto_accident_state=StateOrProvince.parse(x, using=llm_parser),
        accident_job_related_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        accident_death_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        entered_by=XCN(
            id_number=ST(x),
            family_name=FN(
                surname=Lastname.parse(x, using=llm_parser),
        ...
        brought_in_by=ST(x),
        police_notified_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
...
async def pda_26(x: UpstreamInput) -> PDA:
    ...
    pda = PDA(
        ...
        death_certified_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        autopsy_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
        ...
        coroner_indicator=YesOrNoIndicator.parse(x, using=llm_parser),
    )
