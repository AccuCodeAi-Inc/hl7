...
from hl7.v2_5_1.segments import (
    EVN, MSH, PID, PD1, UB1, DB1, ACC, GT1, ROL, PV2, OBX, UB2, DRG, PDA, SFT, AL1, NK1, PV1, DG1
)
from hl7.v2_5_1.data_types import (
    OCD, MSG, MO, ID, CP, FC, XAD, VARIES, HD, DTM, DLD, SAD, SI, XPN, CWE, UVC, EI, CX, TS, CE, OSP, TX, PL, PT, NM, XTN, JCC, DT, DR, DLN, ST, IS, VID, XCN, XON, FN, CNE
)
from hl7.v2_5_1.tables import (
    EventType, CodingSystem, VisitPriorityCode, NameAssemblyOrder, AdvanceDirectiveCode, PersonLocationType, IdentifierType, LivingWillCode, UniversalIdType, ContactRole, VisitUserCode, DrgPayor, LivingDependency, AmbulatoryStatus, CurrencyCodes, PhoneNumber, AbnormalFlags, PurgeStatusCode, VersionId, VisitIndicator, ModeOfArrivalCode, DiagnosisPriority, MilitaryService, DisabledPersonCode, AdmissionType, ProductionClassCode, NotifyClergyCode, StateOrProvince, AlternateCharacterSets, AlternateCharacterSetHandlingScheme, MessageStructure, ProviderRole, Religion, Precision, BedStatus, TelecommunicationEquipmentType, AddressType, ValueType, CpRangeType, Relationship, MessageType, EmploymentStatus, DegreeOrLicenseOrCertificate, PatientStatusCode, SegmentActionCode, TelecommunicationUseCode, Lastname, ProblemOrGoalActionCode, AllergySeverity, MilitaryRankOrGrade, PatientClass, SpecialProgramCode, ImmunizationRegistryStatus, StudentStatus, LivingArrangement, Race, DiagnosisClassification, EventReason, ObservationResultStatusCodesInterpretation, NatureOfAbnormalTesting, MaritalStatus, AdministrativeSex, JobStatus, ProcessingId, OrganizationalNameType, NameOrAddressRepresentation, PrecautionCode, FirstName, CountryCode, AdmitSource, NameType, EthnicGroup, MilitaryStatus, CheckDigitScheme, State, AdmissionLevelOfCareCode, DiagnosisType, HospitalService, OrganDonorCode, ReAdmissionIndicator, PriceType, City, PublicityCode, ProcessingMode, ZipCode, PatientConditionCode, DrgTransferType, OrganizationUnitType, YesOrNoIndicator, AcceptOrApplicationAcknowledgmentConditions, AllergenType, DischargeDisposition, IdentityReliabilityCode, RecreationalDrugUseCode, OutlierType
)
# fmt: on
...
async def pid_4(x: UpstreamInput) -> PID:
    ...
    pid = PID(
        ...
        phone_number_home=XTN(
            telephone_number=PhoneNumber.parse(
                x, using=llm_parser
            ),
...
@activity.defn(name="build :: ADT_A08 => ROL")
async def rol_None(x: UpstreamInput) -> ROL:
    """
    The role segment contains the data necessary to add, update, correct, and delete from the record persons involved, as well as their functional involvement with the activity being transmitted.
    """
    rol = ROL(
...
class Build_ADT_A08:
    ...
    """
    @workflow.run
    async def run(self, x: UpstreamInput) -> ADT_A08:
        build = partial(
        ...
        adt_a08 = ADT_A08(
            ...
            procedure=(                                                          # rpt:*            
                ADT_A08_PROCEDURE_GROUP(
                    pr1=await build(pr1_16),
                    rol=await build(rol_17),
                ),
                ADT_A08_PROCEDURE_GROUP(
                    pr1=await build(pr1_16),
                    rol=await build(rol_17),
            ...
            insurance=(                                                          # rpt:*            
                ADT_A08_INSURANCE_GROUP(
                    in1=await build(in1_19),
                    in2=await build(in2_20),
                    in3=await build(in3_21),
                    rol=await build(rol_22),
                ),
                ADT_A08_INSURANCE_GROUP(
                    in1=await build(in1_19),
                    in2=await build(in2_20),
